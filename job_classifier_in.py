...
def load_and_preprocess_data():
    """Load and preprocess the dataset with class imbalance analysis"""
    ...
    try:
        ...
        if missing_data.any():
            ...
            data = data.dropna(subset=["combined_description_text", "title","category"])
            logger.info(f"After removing missing values: {data.shape[0]} rows")
        
        data["text"] = data["title"].astype(str) + " " + data["combined_description_text"].astype(str)
        X = data["text"]
        y = data["category"]
        ...
        logger.info(f"Unique job categories: {y.nunique()}")
        logger.info(
            f"Average description length: {X.str.len().mean():.1f} characters")

        return X, y
    except FileNotFoundError:
        logger.error("Dataset file 'job_output.csv' not found!")
        raise
...
def perform_cross_validation(models, X_train_tfidf, y_train, cv_folds=5):
    ...
    for name, model in models.items():
        ...
        try:
            ...
            cv_scores = cross_validate(model, X_train_tfidf, y_train,
            ...
            cv_results[name] = {
                ...
                'f1_macro_mean': np.mean(cv_scores['test_f1_macro']),
                'f1_macro_std': np.std(cv_scores['test_f1_macro']),
                'roc_auc_mean': np.mean(cv_scores['test_roc_auc_ovr']),
                'roc_auc_std': np.std(cv_scores['test_roc_auc_ovr'])
            }

            logger.info(f"{name} CV Results:")
            logger.info(f"  Accuracy: {cv_results[name]['accuracy_mean']:.4f} (±{cv_results[name]['accuracy_std']:.4f})")
            logger.info(f"  F1-Macro: {cv_results[name]['f1_macro_mean']:.4f} (±{cv_results[name]['f1_macro_std']:.4f})")
...
def hyperparameter_tuning(X_train_tfidf, y_train, X_val_tfidf, y_val):
    ...
    xgb_params = {
        ...
        'subsample': [0.8, 1.0],
        'scale_pos_weight': [1, 10, 100]  # For handling class imbalance
    }
...
def train_and_evaluate_models(X_train_tfidf, X_val_tfidf, X_test_tfidf,
    ...
    """Train and comprehensively evaluate models with class balancing"""
    logger.info("Training and evaluating models with class balancing...")

    # Base models with class_weight='balanced' where applicable
    base_models = {
        ...
        "Logistic Regression_SMOTE": ImbPipeline([
            ('sampling', SMOTE(random_state=42)),
            ('classification', LogisticRegression(max_iter=1000, random_state=42))
        ]),
        "Random Forest_UnderSample": ImbPipeline([
    ...
    scale_pos_weight = (len(y_train) - class_counts) / class_counts
    ...
    for name, model in base_models.items():
        ...
        try:
            ...
            if 'SMOTE' in name or 'UnderSample' in name or name == 'XGBoost':
                create_confusion_matrix_plot(y_test, test_pred, label_encoder, name)
...
def train_deep_learning_model_enhanced(X_train_tfidf, X_val_tfidf, X_test_tfidf,
    ...
    """Enhanced deep learning model with class weighting"""
    logger.info("Training enhanced deep learning model with class weights...")
    ...
    try:
        ...
        dl_model = Sequential([
            Dense(256, input_shape=(X_train_dl.shape[1])), 
            BatchNormalization(),
            Activation('relu'),
            Dropout(0.5),
            Dense(128),
        ...
        logger.info("Training neural network with class weights...")
...
def generate_detailed_report(results, cv_results, clustering_results, label_encoder, y_filtered):
    """Generate a detailed performance report with imbalance analysis"""
    logger.info("Generating detailed performance report with class imbalance analysis...")
    ...
    report.append("\nCLASS DISTRIBUTION:")
    report.append("-"*40)
    report.append(f"Most common class: {class_counts.idxmax()} (n={class_counts.max()})")
    report.append(f"Least common class: {class_counts.idxmin()} (n={class_counts.min()})")
    report.append(f"Imbalance ratio: {class_counts.max()/class_counts.min():.1f}:1")
    report.append("")
    ...
    for model, cv_result in cv_results.items():
        ...
        report.append(
            f"  ROC-AUC: {cv_result['roc_auc_mean']:.4f} (+/- {cv_result['roc_auc_std']*2:.4f})")
        report.append("")
    ...
    # Add section about class imbalance handling
    report.append("\nCLASS IMBALANCE HANDLING:")
    report.append("-"*40)
    report.append("The following techniques were applied to handle class imbalance:")
    report.append("1. Class weighting in Logistic Regression and Random Forest")
    report.append("2. SMOTE oversampling for Logistic Regression")
    report.append("3. Random undersampling for Random Forest")
    report.append("4. Class weighting in XGBoost (scale_pos_weight)")
    report.append("5. Class weighting in Neural Network")
    report.append("")
    
    # Add recommendation based on imbalance
    if class_counts.max()/class_counts.min() > 10:
        report.append("⚠️  Strong class imbalance detected. Consider:")
        report.append("- Collecting more samples for minority classes")
        report.append("- Trying advanced oversampling techniques like ADASYN")
        report.append("- Using ensemble methods like BalancedRandomForest")
    else:
        report.append("Class imbalance is moderate. Current handling appears sufficient.")
...
def main():
    ...
    try:
        setup_environment()
        logger.info("Starting Enhanced ML Pipeline with Class Imbalance Handling")
        ...
        generate_detailed_report(
            results, cv_results, clustering_results, label_encoder, y_filtered)
        ...
        joblib.dump(trained_models[best_model_name], "models_in/best_model.pkl")
        joblib.dump(tfidf, "models_in/tfidf_vectorizer.pkl")
        joblib.dump(label_encoder, "models_in/label_encoder.pkl")
